      ...
      <div className="glass-panel" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            ...
            >
              <FileJson size={16} />
              <span className="hide-on-mobile">{t('common.uploadJson')}</span>
            ...
            >
              ...
              <span className="hide-on-mobile">{t('common.import')}</span>
            </button>
            <button
              onClick={() => {
                setConfirmConfig({
          ...
          {activeTab === 'bulk' ? (
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{
                ...
                <div>
                  ...
                  <p style={{ margin: '0 0 0.2rem' }}>{t('common.formatStep1')}</p>
                  <p style={{ margin: '0 0 0.2rem' }}>{t('common.formatStep2')}</p>
                  <p style={{ margin: '0 0 0.5rem' }}>{t('common.formatStep3')}</p>
                  <p style={{ margin: 0, opacity: 0.8, fontStyle: 'italic' }}>{t('common.formatExample')}</p>
                </div>
              </div>
              <textarea
                value={bulkText}
                onChange={e => setBulkText(e.target.value)}
                placeholder={t('deckmanager.bulkImportDesc')}
                style={{
            ...
            <div className="animate-fade-in" style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem' }}>
              ...
              <div style={{ flex: '1 1 400px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.8rem' }}>{t('deckmanager.questionContent')}</label>
                  <textarea
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 0', borderTop: '1px solid var(--glass-border)', borderBottom: '1px solid var(--glass-border)' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', color: 'var(--text-muted)' }}>{t('deckmanager.questionType')}</span>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '1rem' }}>{t('deckmanager.optionsInstruction')}</label>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                    ...
                    {currentCard.options.length < 26 && (
                      ...
                      >
                        <Plus size={18} /> {t('deckmanager.addMoreOptions')}
                      </button>
                    )}
                  </div>
                </div>
                ...
                >
                  <ArrowRight size={20} /> {t('deckmanager.addCardToPreview')}
                </button>
              </div>
              ...
              <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', gap: '1rem', borderLeft: '1px solid var(--glass-border)', paddingLeft: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h3 style={{ margin: 0, fontSize: '0.8rem', fontWeight: 'bold', color: 'var(--text-muted)', textTransform: 'uppercase' }}>{t('deckmanager.addedCards', { count: manualCards.length })}</h3>
                  {manualCards.length > 0 && (
                    <button onClick={() => setManualCards([])} style={{ background: 'none', border: 'none', color: '#f87171', fontSize: '0.75rem', cursor: 'pointer', textDecoration: 'underline' }}>{t('deckmanager.clearAll')}</button>
                  )}
                </div>
                <div className="custom-scrollbar" style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', maxHeight: '450px', overflowY: 'auto', paddingRight: '0.5rem' }}>
                  {manualCards.length === 0 ? (
                    <div style={{ padding: '3rem', border: '1px dashed var(--glass-border)', borderRadius: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.8rem', opacity: 0.4 }}>
                      <BookOpen size={32} />
                      <span style={{ fontSize: '0.8rem' }}>{t('deckmanager.noCardsAdded')}</span>
                    </div>
                  ) : (
